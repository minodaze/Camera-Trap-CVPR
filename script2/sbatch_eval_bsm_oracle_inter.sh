...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/eval_bsm_oracle_inter_lr${LEARNING_RATE}.yaml"

    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/ess/PAS2099/camera-trap-CVPR-logs/oracle_inter/bsm_oracle_inter/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: eval_lora
log_path: /fs/ess/PAS2099/camera-trap-CVPR-logs/oracle_inter/bsm_oracle_inter/${DATASET//\//_}
common_config:
  model: bioclip2
