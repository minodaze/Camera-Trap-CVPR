...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}_eval_on_train_lr${LEARNING_RATE}.yaml"
    mkdir -p "/fs/ess/PAS2099/camera-trap-CVPR-logs/oracle_20/oracle_eval_on_train/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: eval_oracle_ontrain
log_path: /fs/ess/PAS2099/camera-trap-CVPR-logs/oracle_20/oracle_eval_on_train/${DATASET//\//_}
common_config:
  model: bioclip2
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --wandb --eval_only --model_dir "${MODEL_DIR}" --eval_on_train --pretrained_weights bioclip2 --full
done