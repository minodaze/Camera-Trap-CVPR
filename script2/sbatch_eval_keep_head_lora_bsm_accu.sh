...
CONFIG_ROOT="/fs/ess/PAS2099/camera-trap-CVPR-configs/best_accum_eval"
...
# ")
    ...
    mkdir -p "/fs/ess/PAS2099/camera-trap-CVPR-logs/accum_80/best_accum_eval/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: eval_lora
log_path: /fs/ess/PAS2099/camera-trap-CVPR-logs/accum_80/best_accum_eval/${DATASET//\//_}
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --wandb --eval_only --model_dir "${MODEL_DIR}" --skip_head --pretrained_weights bioclip2 --lora_bottleneck 8 --lora_interpolate --lora_alpha ${ALPHA}
done