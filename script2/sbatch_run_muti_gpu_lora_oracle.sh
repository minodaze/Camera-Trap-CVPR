...
#SBATCH --gpus-per-node=4         # Four GPUs per node
...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/lora_oracle_multi_pitzer_lr${LEARNING_RATE}.yaml"

    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/ess/PAS2099/camera-trap-CVPR-logs/oracle_20/lora_oracle_test_per_epoch_multi_pitzer/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: oracle
log_path: /fs/ess/PAS2099/camera-trap-CVPR-logs/oracle_20/lora_oracle_test_per_epoch_multi_pitzer/${DATASET//\//_}
common_config:
  model: bioclip2
...
EOF
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.run --standalone --nproc_per_node=4 run_pipeline.py --c $CONFIG_FILE --distributed --lora_bottleneck 8 --eval_per_epoch --test_per_epoch --save_best_model --pretrained_weights bioclip2 --wandb
