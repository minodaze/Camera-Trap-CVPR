...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/eval_rare_best_accum_lr${LEARNING_RATE}.yaml"
    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}/"
    mkdir -p "/fs/ess/PAS2099/camera-trap-CVPR-logs/rare_best_accum/${DATASET//\//_}/"

    cat <<EOF > $CONFIG_FILE
module_name: eval_lora
log_path: /fs/ess/PAS2099/camera-trap-CVPR-logs/rare_best_accum/${DATASET//\//_}/
