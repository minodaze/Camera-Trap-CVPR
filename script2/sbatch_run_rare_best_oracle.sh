...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/rare_best_oracle_lr${LEARNING_RATE}.yaml"

    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/ess/PAS2099/camera-trap-CVPR-logs/rare_best_oracle/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_accum_lora_bsm
log_path: /fs/ess/PAS2099/camera-trap-CVPR-logs/rare_best_oracle/${DATASET//\//_}
