...
"""
Evaluate 5 "when to adapt" algorithms, but ONLY at selected checkpoints
specified by a (dataset, ckp) allow-list CSV (e.g., the 231 non-zero diffs).

Input allow-list CSV must have:
  - dataset: underscore form, e.g., "APN_APN_K051"
  - ckp:     string like "ckp_8"
"""
...
NP_RESULT_DIR    = "/fs/scratch/PAS2099/Lemeng/icicle/camera-trap-final/np_result_filtered_shouldadapt"
ALLOWLIST_CSV    = "/users/PAS2099/lemengwang/Documents/icicle/Camera-Trap-CVPR/anaylisis_code/always_no_pick_higher_nonzero_diffs.csv"
...
def _u2d(ds_u: str) -> str:
    """'APN_APN_13U' -> 'APN/APN_13U' (best-effort for matching)."""
    return ds_u.replace("_", "/")
...
def class_mean_distance_ref_vs_target(feat_model: nn.Module, ref_loader, target_loader) -> float:
    ...
    # Nearest L2 <=> max dot when both are unit-norm.
    dots = tgt_X @ ref_means.T                 # [N, C]
    assigned = dots.argmax(dim=1)              # [N]
    nearest_proto = ref_means[assigned]        # [N, D]

    # ---- Ensure both sides are normalized before distance (redundant but explicit) ----
    tgt_X        = _l2_normalize_rows(tgt_X)
    nearest_proto= _l2_normalize_rows(nearest_proto)
...
def run_for_dataset(dataset: str, rng: random.Random, selected_Ts: Set[int]) -> Dict[str, object]:
    ...
    Per-ckp CSV will only contain rows for selected_Ts.
    """
    ...
    if not chosen_Ts:
        return {"summary": {k: float("nan") for k in ["random","always_yes","always_no","ood_msp","class_mean","pick_higher"]},
                "per_ckp": pd.DataFrame(columns=["ckp","random","always_yes","always_no","ood_msp","class_mean","pick_higher"])}
...
def main():
    ...
    for ds in tqdm(DATASETS, desc="Datasets"):
        try:
            ...
            if not selected_Ts:
                # Skip datasets not present in allow-list
            ...
            # write per-ckp CSV per dataset (only selected rows)
    ...
    # summary.csv (restricted means)
    df_summary = pd.DataFrame.from_dict(per_dataset_summary, orient="index")
    df_summary.index.name = "dataset"
    df_summary.reset_index(inplace=True)
    df_summary.to_csv("./when_to_adapt_out/summary_needTrain.csv", index=False)
    print('Saved per-dataset means to ./when_to_adapt_out/summary_needTrain.csv')

    # report how many pairs we actually evaluated
    print(f"✅ Evaluated exactly {used_pairs_counter} (dataset, ckp) rows from the allow-list.")

    # (optional) print to console
    print("\nPer-dataset means (restricted):")
