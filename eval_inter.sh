...
readarray -t ALL_DATASETS < uselist/oracle_inter.txt

# Read model directories from model_dirs.txt (one directory per line)
readarray -t MODEL_DIRS < uselist/eval_lora_oracle_inter_model_path.txt
        ...
        for i in "${!job_datasets[@]}"; do
            ...
            if [ -f "$json_path" ]; then
                ...
                for r in "${ratio[@]}"; do
                    ...
                    # sbatch script2/sbatch_eval_best_accu_inter.sh "${dataset}" "$lr" "${model_dir}" "$r"
                    # sbatch script2/sbatch_eval_oracle_inter.sh "${dataset}" "$lr" "${model_dir}" "$r"
                    sbatch script2/sbatch_eval_lora_oracle_inter.sh "${dataset}" "$lr" "${model_dir}" "$r"
                    # sbatch script2/sbatch_eval_bsm_oracle_inter.sh "${dataset}" "$lr" "${model_dir}" "$r"
                    # sbatch script2/sbatch_eval_best_oracle_inter.sh "${dataset}" "$lr" "${model_dir}" "$r"
