...
def CB_loss(logits, labels, samples_per_cls, no_of_classes, loss_type, beta, gamma, device, use_per_class_beta=False):
    ...
    if loss_type == "focal":
        ...
        # Apply CB weighting
        weighted_bce = weights * bce  # [B, C]

        # Normalize: sum over elements, divide by number of positive labels
        cb_loss = weighted_bce.sum() / labels_one_hot.sum()
    elif loss_type == "softmax":
        ce = F.cross_entropy(logits, labels, reduction="none")
        cb_loss = torch.mean(weights[:,0] * ce)
        # pred = logits.softmax(dim = 1)
        # cb_loss = F.binary_cross_entropy(input = pred, target = labels_one_hot, weight = weights)
    
    return cb_loss

def cdt_loss(logits, labels, samples_per_cls, gamma=0.3, device='cuda'):
    """
    Unified CDT loss via scaled logits + F.cross_entropy.
    
    If gamma=0, temperatures all =1, so this is exactly F.cross_entropy(logits, labels).
    """
    ...
    temperatures = (N_max / samples_per_cls) ** gamma  # [C]
    # 3) 缩放 logits
    scaled_logits = logits / temperatures.unsqueeze(0)   # [batch, C]
    # 4) 直接调用 cross_entropy
...
def per_sample_balanced_softmax_loss(logits, labels, samples_per_cls, no_of_classes, device='cuda'):
    """Compute the Balanced Softmax Loss.
    ...
    # Compute balanced softmax loss using the reweighted logits
    bsl_loss = F.cross_entropy(reweighted_logits, labels, reduction="none")
