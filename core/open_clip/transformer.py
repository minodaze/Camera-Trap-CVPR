...
class Attention(nn.Module):
    def __init__(
        ...
        if params is not None and params.lora_bottleneck > 0:
...
class CustomResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        # PEFT module initialization - adapter, convpass, repadapter
        self.params = params
        if params is not None:
            if params.ft_attn_module:
                self.ft_attn_module = PEFT_MODULE_REGISTRY[params.ft_attn_module](dim=d_model, params=params)
            else:
                self.ft_attn_module = None
            if params.ft_mlp_module:
                self.ft_mlp_module = PEFT_MODULE_REGISTRY[params.ft_mlp_module](dim=d_model, params=params)
            else:
                self.ft_mlp_module = None
        else:
            self.ft_attn_module = None
            self.ft_mlp_module = None

    def _forward_helper(self, x, norm, ft_module, ls, main, order):
        if ft_module:
            if order == 1:
                ft_mode = self.params.ft_attn_mode
                ft_ln = self.params.ft_attn_ln
            elif order == 2:
                ft_mode = self.params.ft_mlp_mode
                ft_ln = self.params.ft_mlp_ln
        
            if ft_mode == 'parallel':
                if ft_ln == 'before':
                    return x + ls(ft_module(x)) + ls(main(norm(x)))
                elif ft_ln == 'after':
                    return x + ls(ft_module(norm(x))) + ls(main(norm(x)))
                else:
                    raise NotImplementedError
            elif ft_mode == 'sequential_after':
                return x + ls(ft_module(ls(main(norm(x))), add_residual=True))
            elif ft_mode == 'sequential_before':
                return x + ls(main(ft_module(norm(x))))
            else:
                raise NotImplementedError
        else:
            return x + ls(main(norm(x)))
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        # MLP path
        x = self._forward_helper(
            x, self.ln_2, self.ft_mlp_module, self.ls_2, self.mlp, 2
        )
