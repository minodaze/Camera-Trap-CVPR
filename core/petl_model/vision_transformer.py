...
## added for petl
from .block import BlockPETL
from .patch_embed import PatchEmbedPETL
...
class VisionTransformerPETL(VisionTransformer):
    ...
    def load_pretrained(self, checkpoint_path: str, prefix: str = '', model_type: str= '') -> None:
        if 'clip' in model_type:
            ...
            raise ValueError(f'Unsupported model type {model_type} for loading pretrained weights.')
...
def _load_weights_pth(model, checkpoint_path, filter_fn=checkpoint_filter_fn):
    ...
    for k, v in raw.items():
        if k.startswith('model.visual.'):
            k = k.replace('model.', '', 1)        # →  visual.……
        elif not k.startswith('visual.'):
            continue                              # skip text tower & logit scale
        visual_sd[k] = v

    proj_w = visual_sd.get('visual.proj', None)

    if filter_fn is not None:
        visual_sd = filter_fn(visual_sd, model)
    
    if proj_w is not None:
        visual_sd['proj.weight'] = proj_w.t().contiguous()

    if 'head.weight' in visual_sd:
        visual_sd.pop('head.weight', None)
    if 'head.bias' in visual_sd:
        visual_sd.pop('head.bias', None)
