...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/upper_bound_lr${LEARNING_RATE}.yaml"

    # Create the dataset-specific directory
    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/scratch/PAS2099/camera-trap-final/dataAnalysis_logs/${DATASET//\//_}/upper_bound_lora_ce_loss/"

    cat <<EOF > $CONFIG_FILE
module_name: upper_bound
log_path: /fs/scratch/PAS2099/camera-trap-final/dataAnalysis_logs/${DATASET//\//_}/upper_bound_lora_ce_loss/
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --plot_features --wandb --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --lora_bottleneck 8
