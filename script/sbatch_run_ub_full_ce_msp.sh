...
# ")
        ...
        CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/upper_bound_bsm_randomOOD_lr${LEARNING_RATE}_frac${FRACTION}.yaml"
        # Create the dataset-specific directory
        mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
        mkdir -p "/fs/scratch/PAS2099/camera-trap-final/dataAnalysis_logs/${DATASET//\//_}/upper_bound_FT_ce_loss_frac${FRACTION}/"

        cat <<EOF > $CONFIG_FILE
module_name: upper_bound
log_path: /fs/scratch/PAS2099/camera-trap-final/dataAnalysis_logs/${DATASET//\//_}/upper_bound_FT_ce_loss_frac${FRACTION}/
...
EOF
        ...
        python run_pipeline.py --c $CONFIG_FILE --plot_features --wandb --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --full
    done
