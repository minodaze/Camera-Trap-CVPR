...
# ")
        ...
        CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/upper_bound_ce_randomOOD_lr${LEARNING_RATE}_frac${FRACTION}.yaml"
        # Create the dataset-specific directory
        mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
        mkdir -p "/fs/scratch/PAS2099/camera-trap-final/randomOOD_logs/${DATASET//\//_}/upper_bound_ce_loss_frac${FRACTION}/"

        cat <<EOF > $CONFIG_FILE
module_name: upper_bound
log_path: /fs/scratch/PAS2099/camera-trap-final/randomOOD_logs/${DATASET//\//_}/upper_bound_ce_loss_frac${FRACTION}/
...
ood_config:
  method: cls_random
  random_fraction: ${FRACTION}  # Fraction of OOD samples to include during training
