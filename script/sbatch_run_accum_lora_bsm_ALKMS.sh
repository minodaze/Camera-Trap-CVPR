...
# ")
        ...
        CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/accum_lora_bsm_ALKMS_lr${LEARNING_RATE}_percentage${FRACTION}.yaml"
        # Create the dataset-specific directory
        mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
        mkdir -p "/fs/scratch/PAS2099/camera-trap-final/AL_logs/${DATASET//\//_}/accum_lora_bsm_loss/"

        cat <<EOF > $CONFIG_FILE
module_name: accumulative
log_path: /fs/scratch/PAS2099/camera-trap-final/AL_logs/${DATASET//\//_}/accum_lora_bsm_loss/
...
EOF
        ...
        python run_pipeline.py --c $CONFIG_FILE --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --lora_bottleneck 8
    done
