...
# ")
    ...
    cat <<EOF > $CONFIG_FILE
module_name: eval_lora_interpolation
log_path: /fs/scratch/PAS2099/camera-trap-final/eval_logs/${DATASET//\//_}/eval_lora_bsm_ub_interpolation/
...
EOF
    ...
    for alpha in "${interpolation_alpha[@]}"; do
        ...
        python run_pipeline.py --c $CONFIG_FILE --eval_only --model_dir "${MODEL_DIR}" --pretrained_weights bioclip2 --lora_bottleneck 8 --merge_factor ${alpha}
    done
