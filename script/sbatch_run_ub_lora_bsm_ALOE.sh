...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    for FRACTION in "${fractions[@]}"; do
        echo "=== Processing ${DATASET} with fraction ${FRACTION} ==="
        # === Generate deterministic timestamp based on dataset, learning rate, and fraction ===
        # This ensures identical runs use the same directory, improving reproducibility
        HASH_INPUT="${DATASET}_${LEARNING_RATE}_${FRACTION}"
...
# ")
        ...
        CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/upper_bound_lora_bsm_ALOE_lr${LEARNING_RATE}_percentage${FRACTION}_sample_per_cluster_${num_sample_per_cluster}.yaml"
        # Create the dataset-specific directory
        mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
        mkdir -p "/fs/scratch/PAS2099/camera-trap-final/dataAnalysis_logs/${DATASET//\//_}/upper_bound_lora_bsm_loss/"

        cat <<EOF > $CONFIG_FILE
module_name: upper_bound
log_path: /fs/scratch/PAS2099/camera-trap-final/dataAnalysis_logs/${DATASET//\//_}/upper_bound_lora_bsm_loss/
...
al_config:
  method: aloe
  percentage: ${FRACTION}
  num_sample_per_cluster: ${num_sample_per_cluster}

cl_config:
  method: none
EOF

        echo "Running pipeline for ${DATASET} with LR=${LEARNING_RATE} and fraction=${FRACTION}"
        python run_pipeline.py --c $CONFIG_FILE --plot_features --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --lora_bottleneck 8
    done
