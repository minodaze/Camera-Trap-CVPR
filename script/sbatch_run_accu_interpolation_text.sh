...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}_accumulative-scratch_lr${LEARNING_RATE}.yaml"

    cat <<EOF > $CONFIG_FILE
module_name: accumulative-scratch
log_path: /fs/scratch/PAS2099/camera-trap-final/logs/${DATASET//\//_}/accum_ce_loss/
...
EOF
    ...
    for alpha in "${interpolation_alpha[@]}"; do
        ...
        python run_pipeline.py --c $CONFIG_FILE --wandb --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --text full --interpolation_model --interpolation_alpha ${alpha} --full
    done
