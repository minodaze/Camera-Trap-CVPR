...
NP_RESULT_DIR    = "/fs/scratch/PAS2099/Lemeng/icicle/camera-trap-final/np_result_feature_saving"
...
SUMMARY_ROOT = "/fs/scratch/PAS2099/Lemeng/icicle/camera-trap-final/np_result_feature_saving/when_to_adapt_out"  # per-dataset folders live here
...
def run_for_dataset(dataset: str, rng: random.Random) -> Dict[str, object]:
    ...
    try:
        ...
        tau_msp_local = float(np.mean(compute_msp_scores(model_1, eval_loader_1)))
    except Exception as e:
    ...
    for T in trange(2, max(available_Ts)+1, desc=f"{_d2u(dataset)} | full eval", leave=False):
        ...
        try:
            ...
            if T == 2:
                ...
                decision_yes_msp = (msp_mean <= tau_msp_local)
            print(f"[{dataset}] T={T}: MSP mean={msp_mean:.4f} vs τ_msp={tau_msp_local:.4f} -> decision_yes={decision_yes_msp}")
            ...
            with open(os.path.join(out_dir, "msp_meta.json"), "w") as f:
                json.dump({
                    ...
                    "tau_msp_in_use": float(tau_msp_local),
            ...
            try:
                ...
                with open(os.path.join(out_dir, "feat_meta.json"), "w") as f:
                    json.dump({
                        ...
                        "TAU_FEAT_used": float(TAU_FEAT)
                    }, f, indent=2)

                decision_yes_feat = (dist_mean > TAU_FEAT) if not math.isnan(dist_mean) else False
