...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/zs_30_openai/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: zs
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/zs_30_openai/${DATASET//\//_}
common_config:
  model: bioclip2
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --text_templete bioclip --wandb --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --full --loss_type ce
