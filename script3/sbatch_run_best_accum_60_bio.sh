...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    TRAIN_JSON="${DATA_ROOT}/${DATASET}/60/train.json"
    TEST_JSON="${DATA_ROOT}/${DATASET}/60/test.json"
    ALL_JSON="${DATA_ROOT}/${DATASET}/60/train-all.json"
...
# ")
    ...
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_accum_60_bio/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_accum_lora_bsm
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_accum_60_bio/${DATASET//\//_}
