...
# ")
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/best_oracle_lr${LEARNING_RATE}.yaml"

    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/all_class_best_oracle/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_accum_lora_bsm
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/all_class_best_oracle/${DATASET//\//_}
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --resume --wandb --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --expand_head /users/PAS2099/mino/ICICLE/unique_species.txt --lora_bottleneck 8 --loss_type bsm
