...
DATA_ROOT="/fs/scratch/PAS2099/camera-trap-benchmark/dataset_15_60"
...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    TRAIN_JSON="${DATA_ROOT}/${DATASET}/15/train.json"
    TEST_JSON="${DATA_ROOT}/${DATASET}/15/test.json"
    ALL_JSON="${DATA_ROOT}/${DATASET}/15/train-all.json"
...
# ")
    ...
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_accum_15_bio/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_accum_lora_bsm
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_accum_15_bio/${DATASET//\//_}
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --text_template bioclip --resume --wandb --eval_per_epoch --save_best_model --pretrained_weights bioclip2 --lora_bottleneck 8 --loss_type bsm
