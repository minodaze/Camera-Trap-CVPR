...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_oracle_wildclip_openai/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_accum_lora_bsm
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_oracle_wildclip_openai/${DATASET//\//_}
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --resume --wandb --eval_per_epoch --save_best_model --pretrained_weights wildclip --lora_bottleneck 8 --loss_type bsm
