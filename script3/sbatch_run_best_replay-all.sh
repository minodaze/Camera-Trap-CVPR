...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/best_replay_lr${LEARNING_RATE}.yaml"

    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_replay_all_openai/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_replay_lora_bsm
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_replay_all_openai/${DATASET//\//_}
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --wandb --eval_per_epoch --test_per_epoch --save_best_model --device cuda:0 --pretrained_weights bioclip2 --loss_type bsm --lora_bottleneck 8
done