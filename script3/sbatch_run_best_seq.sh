...
for DATASET in "${BIG_FOLDERS[@]}"; do
    ...
    CONFIG_FILE="${CONFIG_ROOT}/${DATASET//\//_}/best_seq_lr${LEARNING_RATE}.yaml"

    mkdir -p "${CONFIG_ROOT}/${DATASET//\//_}"
    mkdir -p "/fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_seq_openai_new/${DATASET//\//_}"

    cat <<EOF > $CONFIG_FILE
module_name: best_seq_lora_bsm
log_path: /fs/scratch/PAS2099/camera-trap-ECCV/ascend3/best_seq_openai_new/${DATASET//\//_}
...
EOF
    ...
    python run_pipeline.py --c $CONFIG_FILE --wandb --pretrained_weights bioclip2 --loss_type bsm --lora_bottleneck 8
done