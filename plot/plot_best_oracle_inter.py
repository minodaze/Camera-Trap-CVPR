...
with open('/users/PAS2099/mino/ICICLE/eval_best_oracle_dataset.txt', 'r') as file:
    datasets = [l.strip() for l in file.read().splitlines() if l.strip() and not l.strip().startswith('#')]

# Ratios from 0.0 to 1.0 (inclusive)
ratios = [round(0.1 * i, 2) for i in range(1, 10)]

base_dir = '/fs/ess/PAS2099/camera-trap-CVPR-logs/accum_80/best_oracle_inter/'
...
def load_ba_for(ds: str, ratio: float):
    ...
    # camera-trap-CVPR-logs/oracle_20/best_oracle_inter/ENO_ENO_D06/bioclip2/full_text_head_interpolation_model_0.0/log
...
plt.savefig('plots2/best_oracle_interpolation_all_datasets.png', dpi=200)
plt.close()
...
for idx, row in df.iterrows():
    ...
    df.loc[idx, 'best oracle + best inter ratio'] = value

df.to_csv('csv/camera-trap-CVPR - (Correct best oracle inter best ratio) lora-inter best ratio Final Best Accum List.csv', index=False)
print(f"Updated CSV with interpolation results to csv/camera-trap-CVPR - (Correct best oracle inter best ratio) lora-inter best ratio Final Best Accum List.csv.")
