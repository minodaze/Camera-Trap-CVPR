...
for lr in "${LEARNING_RATES[@]}"; do
    ...
    for ((job_index=0; job_index<TOTAL_JOBS; job_index++)); do
        ...
        sbatch script2/sbatch_run_best_replay.sh "${datasets_string}" "$lr"
        # sbatch script2/sbatch_run_best_derpp.sh "${datasets_string}" "$lr"
        # sbatch script2/sbatch_run_best_mir.sh "${datasets_string}" "$lr"
        # sbatch script2/sbatch_run_best_randrepold.sh "${datasets_string}" "$lr"
    done
done
